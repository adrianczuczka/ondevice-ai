...
public class OnDeviceAi(
    ...
    public suspend fun engineStatuses(feature: AiFeature = AiFeature.Prompt): Map<String, Availability> =
        statusMap(feature)

    /** Fetches weights for the first engine that reports [Availability.Downloadable]. */
    public fun download(feature: AiFeature = AiFeature.Prompt): Flow<DownloadProgress> = flow {
    ...
    public suspend fun chat(config: SessionConfig = SessionConfig()): ChatSession =
        selectEngine().openSession(config)

    /** One-shot generation over a throwaway session. */
    public suspend fun generate(prompt: String, options: GenerationOptions = GenerationOptions()): String =
