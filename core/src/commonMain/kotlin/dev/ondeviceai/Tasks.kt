...
enum class SummaryType { Bullets, Paragraph }

enum class Tone { Professional, Friendly, Elaborate, Shorten }
...
suspend fun OnDeviceAi.summarize(
    text: String,
...
suspend fun OnDeviceAi.proofread(text: String): String =
    generate(
...
suspend fun OnDeviceAi.rewrite(text: String, tone: Tone = Tone.Professional): String {
...
suspend fun OnDeviceAi.describeImage(image: AiImage): String {
    TODO(
        "Image input lands with the ML Kit ImageDescription / Prompt image-part " +
            "wiring on Android; not yet designed for iOS."
    )
}
